...
public class TestRequesterService extends TestBase {
  ...
  public void testCreateExternalQuestion() throws Exception {
    HITQuestion htmlQuestion = new HITQuestion(defaultExternalQuestionFileName);
    HIT[] hits = service.createHITs(defaultHITInput, defaultHITProperties, 
            htmlQuestion, null, null);

    assertNotNull(hits);
    assertTrue("Should have created more than one HIT, but created " + hits.length, hits.length > 1);
    assertNotNull(hits[1]);
    assertNotNull(hits[1].getHITId());
  }
  
  public void testCreateInvalidExternalQuestion() throws Exception {
    try {
        QAPValidator.validateFile(defaultInvalidExternalQuestionFileName);
        fail("Expected ValidationException when creating HIT with invalid external question.");
    }
  ...
  public void testValidateInvalidHTMLQuestion() throws Exception {
    ...
    try {
      ...
      fail("Expected ValidationException when validating a HIT with an invalid HTMLQuestion");
    } catch (ValidationException e) {
      // Expected exception
  ...
  public void testCreateInvalidFormattedContentHIT() throws Exception {
      try {
        QAPValidator.validateFile(defaultInvalidFormattedContentQuestionFileName);
  ...
  public void testCreateScriptFormattedContentHIT() throws Exception {
    try {
      QAPValidator.validateFile(defaultScriptFormattedContentQuestionFileName); 
